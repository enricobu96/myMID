...
class Scene(object):
    ...
    Args:
        ...
        map: object (None) : map for visualization purposes
        dt: float : hyperparameter, dt of the time for derivation of space (in order to get velocity and acceleration)
        name: str : name of the dataset
        frequency_multiplier : int : multiplier for the frequency when operating in node_freq_mult
        aug_func: function : augmentation function
        non_aug_scene: Scene : non-augmented scene
        robot: Node : robot node when operating with it
        temporal_scene_graph: SceneGraph : see SceneGraph documentation
        description: str : description of the dataset

    Methods:
        add_robot_from_nodes(robot_type): None : appends robot node to nodes when operating with robot node
        get_clipped_pos_dict(timestep,state): dict() : returns nodes clipped at time
        get_scene_graph(timestep,attention_radius=None,edge_addition_filter=None,edge_removal_filter=None): SceneGraph : returns the Scene Graph for a given timestep. Read documentation of the method
        calculate_scene_graph(attention_radius,edge_addition_filter=None,edge_removal_filter=None): None : calculate the Temporal Scene Graph for the entire Scene.
        duration(): int : returns the duration of the scene
        present_nodes(timesteps,type=None,min_history_timesteps=0,min_future_timesteps=0,return_robot=True): dict : finds all present nodes in the scene at a given timestemp
        get_nodes_clipped_at_time(timesteps,state): list(Nodes) : clips nodes at time
        sample_timesteps(batch_size,min_future_timesteps=0): np.ndarray : sample a batch size of possible timesteps for the scene
        augment(): self : performs augmentation
